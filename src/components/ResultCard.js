...
const ResultCard = ({ result, onResultClick, onDeleteResult }) => {
  ...
  return (
    <motion.div 
      ...
      className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-3 m-2 flex-shrink-0 w-full sm:w-1/3 md:w-1/3 lg:w-1/3 xl:w-1/3 transition-all duration-300 hover:bg-white/15 hover:border-white/30 shadow-2xl hover:shadow-white/10 flex flex-col relative group"
    >
      ...
      <button
        ...
        className="absolute top-2 right-2 w-6 h-6 bg-red-500/80 hover:bg-red-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10"
        title="Delete result"
      >
